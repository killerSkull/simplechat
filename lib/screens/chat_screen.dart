...
class ChatScreen extends StatefulWidget {
  ...
  const ChatScreen({super.key, required this.otherUser, this.nickname});
...
class _ChatScreenState extends State<ChatScreen> with WidgetsBindingObserver {
  ...
  void initState() {
    ...
    final chatId = _firestoreService.getChatId(currentUser.uid, widget.otherUser.uid);
    ...
    _checkIfContact();
    _firestoreService.markMessagesAsRead(chatId: chatId, currentUserId: currentUser.uid);
  ...
  void _listenForChatUpdates(String chatId, String currentUserId) {
    _chatSubscription = _firestoreService.getChatStream(chatId).listen((chatSnapshot) {
      if (mounted) {
        ...
        if (chatSnapshot.exists) {
          ...
          final newClearedAtTimestamp = chatData[clearedAtField] as Timestamp?;
          if (newClearedAtTimestamp != null) {
            newQuery = newQuery.where('timestamp', isGreaterThan: newClearedAtTimestamp);
          }
        }
        setState(() => _messagesQuery = newQuery);
      }
    });
  }
  ...
  void _onTextChanged(String text) {
    _typingTimer?.cancel();
    if (text.isNotEmpty) {
      _firestoreService.updateTypingStatus(chatId: _chatManager.chatId, currentUserId: _chatManager.currentUserId, isTyping: true);
      _typingTimer = Timer(const Duration(seconds: 2), () {
        _firestoreService.updateTypingStatus(chatId: _chatManager.chatId, currentUserId: _chatManager.currentUserId, isTyping: false);
      });
    } else {
      _firestoreService.updateTypingStatus(chatId: _chatManager.chatId, currentUserId: _chatManager.currentUserId, isTyping: false);
    }
  ...
  Future<void> _stopRecording() async {
    ...
    if (path != null) {
      ...
      final uploadTask = _storageService.uploadChatAudio(chatId: _chatManager.chatId, filePath: path);
      _chatManager.handleFileUpload(uploadTask, message, audioDuration: _recordingDuration.inMilliseconds);
    }
  }
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: const Text('¿Estás seguro de que quieres eliminar todos los mensajes de este chat solo para ti?'),
          actions: <Widget>[
            TextButton(
            ...
            TextButton(
              ...
              onPressed: () async {
                await _firestoreService.clearChatForUser(chatId: _chatManager.chatId);
  ...
  Future<void> _exportChat() async {
    final currentUserDoc = await _firestoreService.db.collection('users').doc(_chatManager.currentUserId).get();
    ...
    await Share.shareXFiles([XFile(file.path)], text: 'Historial de chat de SimpleChat');
  }
          ...
          children: [
            Expanded(
              child: MessageList(
                ...
                selectedMessages: _chatManager.selectedMessages.map((doc) => doc.id).toList(),
                // --- CORRECCIÓN AQUÍ: Se añade el parámetro que faltaba ---
                onAddContact: (uid, name) {
                  _firestoreService.addContact(uid, name);
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(content: Text('$name ha sido añadido a tus contactos.')),
                  );
                },
              ),
            ),
            if (!_isSearching && !_chatManager.isSelectionMode)
              MessageInputBar(
                ...
                onSendMedia: () => showAttachmentMenu(
                  ...
                  onPickContact: () => _chatManager.showContactPicker(widget.otherUser.uid),
                ...
                onAddContact: () async {
                  await _firestoreService.addContact(widget.otherUser.uid, widget.otherUser.displayName ?? '');
                   _checkIfContact();
                },
              ),
          ],
